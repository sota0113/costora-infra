...
| GPU vs CPU | Bedrock移行によりGPU不要に | [ADR 008](docs/adr/008-bedrock-migration.md) |
| インスタンスサイズ | 低負荷ワークロードのため t3.micro に縮小 | [ADR 009](docs/adr/009-ec2-downsize-to-micro.md) |
...
flowchart TD
    ...
    subgraph apne1["AWS ap-northeast-1"]
        route53["Route53\ncostora.net"]
        eip["EIP\n固定パブリックIP"]
    ...
    sender -->|SMTP| ses
    ses -->|保存| s3mail
    s3mail -->|S3イベント| lambda
    lambda -->|"POST /api/webhook/ses-invoice\nX-Webhook-Secret"| nextjs

    user -->|HTTPS| nextjs
    nextjs -->|"HTTPS X-Api-Key\nhttps://inference.costora.net"| nginx

    route53 -->|"A record"| eip
    eip --> nginx
    nginx -->|proxy_pass| fastapi
    fastapi -->|"bedrock:InvokeModel\n(Instance Profile 認証)"| bedrock
    fastapi -->|ファイル保存| s3inv
    nextjs -->|APIキー管理| dynamo
```
...
| リソース | 用途 |
|---|---|
| `aws_instance.ollama` | 推論サーバー (t3.micro、nginx + FastAPI) |
