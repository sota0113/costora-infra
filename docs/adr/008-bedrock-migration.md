...
- **EC2**: GPU 不要となるため g4dn.xlarge → t3.small に縮小
- **AMI**: Deep Learning AMI → Amazon Linux 2023 標準 AMI に変更
- **ボリューム**: モデルウェイト不要のため 50GB → 20GB に縮小
...
- **コスト**: EC2 $0.526/h (g4dn.xlarge) → $0.0208/h (t3.small) に削減。Bedrock 推論コストが加算されるが、呼び出し頻度が低ければ大幅な削減になる
