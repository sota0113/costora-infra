...
- **IAM**: EC2 インスタンスロールに `bedrock:InvokeModel` 権限を付与。`us.` 推論プロファイルは us-east-1 / us-east-2 / us-west-2 へクロスリージョンルーティングするため、3リージョン分の foundation-model ARN を許可する（1リージョンのみだと別リージョンへルーティングされた際に `AccessDeniedException` になる）
